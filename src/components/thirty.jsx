...
export const Thirty = () => {
  ...
  const [success, setSuccess] = useState(false); // Состояние для отображения сообщения об успешной отправке
  ...
  const onSubmit = (e) => {
    ...
    send(
      ...
      .then((response) => {
        ...
        setSuccess(true); // Установка состояния success в true при успешной отправке
      })
      .catch((err) => {
      ...
      <div className="bg-[#fff] rounded-md w-[550px] p-8 m-20 shadow-md shadow-gray-400 sm:m-0">
        ...
        {success ? (
          <div>
            ...
            <p className="font-light text-gray-500">Ожидайте ответа на вашу почту в течении 24ч</p>
          </div>
        ) : (
          <form onSubmit={onSubmit} className="my-8 leading-9">
            ...
            <button type="submit" value="Send Request" className='bg-[#e09a8e] hover:bg-[#fa9684] duration-300 rounded-full px-7 py-3 text-[#fff] text-[17px]'>
              Отправить
            </button>
          </form>
        )}
